...
% % Span of the filter.
% span = 2 * N;
% imgFiltered = zeros(height, width);
% for x = 1:height
%     for y = 1:width
%         % Extract pixel with its neighbors for computation.
%         kernel = imgpad(x:x+span, y:y+span);
...
%         [SRE, LRE, GLN, RLN, RP, LGRE, HGRE, SRLGE, SRHGE, LRLGE, LRHGE] = featureGLRLM(kernel);
% %         [coa, con, dir] =featureTamura(kernel);
% %         imgFiltered(x, y) = GLN(1);
%         if img(x, y) == 0
%             imgFiltered(x, y) = 0;
%         else
%             imgFiltered(x, y) = RLN(1);
%         end
%     end
% end
% 
% % disp(max(imgFiltered, [], 'all'));
% imgFiltered = imgFiltered / max(imgFiltered, [], 'all');
% imwrite(imgFiltered, strcat('C:\Users\Zhe\Desktop\TextureComputation\Matlab\test\', "test.png"))
% 
% 
% disp(all);
% imshow(imgFiltered);
